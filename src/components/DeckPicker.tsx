...
export function DeckPicker({ lessons, lesson, onSelect, onImport, level, onLevel }: Props) {
  const [details, setDetails] = useState(false)
  return (
    <div className="deck-panel">
      <div className="deck-panel-heading">
        <span className="eyebrow">我的卡组</span>
        <button className="text-button" type="button" onClick={onImport}>＋ 添加 Anki 卡组</button>
      </div>
      <label className="field-label" htmlFor="deck-select">选择卡组</label>
      <select id="deck-select" value={lesson.id} onChange={(event) => onSelect(event.target.value)}>
      ...
      <select id="level-select" value={level} onChange={(event) => onLevel(event.target.value)}>
